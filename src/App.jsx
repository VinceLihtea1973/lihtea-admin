...
// === Users Management ===
function Users(){
  const[users,sU]=useState([]);const[loading,sL]=useState(true);const[m,sM]=useState(null);const[t,sT]=useState("");const[f,sF]=useState({});const[pwModal,sPW]=useState(null);const[newPw,sNPw]=useState("");
  const F=(k,v)=>sF(p=>({...p,[k]:v}));const fl=x=>{sT(x);setTimeout(()=>sT(""),3000)};
...
// === Taux de Financement ===
function TauxFin(){
  const{data,loading,create,update,remove}=useCrud("taux_financement");const[m,sM]=useState(null);const[t,sT]=useState("");const[f,sF]=useState({});
  const F=(k,v)=>sF(p=>({...p,[k]:v}));const fl=x=>{sT(x);setTimeout(()=>sT(""),3000)};
  return<div><Toast msg={t}/>
    ...
    <DT loading={loading} data={data} onEdit={r=>{sF({...r});sM("edit")}} onDelete={r=>{if(confirm("Supprimer ce taux?"))remove(r.id).then(()=>fl("✓"))}} columns={[
